...
class DualDistilTrainCollator:
    ...
    def __call__(self, features: List[Tuple[str, List[str]]]):
        all_queries = [f[0] for f in features]
        all_passages = []
        all_subqueries, num_subqueries, all_reranker_scores = [], [], []
        for i, f in enumerate(features):
            n_sq = len(all_subqueries)
            all_passages.extend(f[1])
            all_subqueries.extend(f[2])
            num_subqueries.append(len(f[2]))
            all_reranker_scores.append(f[3])
        ...
        # The distillation scores are at the end.
        all_reranker_scores = torch.tensor(all_reranker_scores, dtype=self.torch_dtype)
        return q_collated, d_collated, sq_collated, num_subqueries, all_reranker_scores
