...
from tevatron.retriever.dataset import Dataset
...
class QrelDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.eval_data = load_dataset(
            self.data_args.eval_dataset_name if dataset_name is None else dataset_name,
            split=self.data_args.eval_dataset_split,
    ...
    def __getitem__(self, item):
        ...
        # Select negative document ids
        negative_size = self.data_args.eval_group_size - 1
        if len(negative_document_ids) < negative_size:
            selected_negative_docids = random.choices(negative_document_ids, k=negative_size)
        elif self.data_args.eval_group_size == 1:
            selected_negative_docids = []
        else:
            offset = epoch * negative_size % len(negative_document_ids)
            selected_negative_docids = list(negative_document_ids)
